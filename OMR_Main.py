...
if not os.path.exists('Scanned'):
    os.makedirs('Scanned')#sonradan ekledim
# Temel ayarlar
webCamFeed = True  # Kameradan canlı besleme kullanılacak mı?
pathImage = "4.jpg"  # Kullanılacak görüntünün yolu
cap = cv2.VideoCapture(0)  # Kamerayı başlat, verilen parametre kamerea sayısını belirler
cap.set(10,160)  # Kameranın parlaklık ayarını yap
heightImg = 700  # Görüntünün yüksekliği
widthImg = 700   # Görüntünün genişliği
questions = 5    # Soru sayısı
choices = 5      # Her soru için seçenek sayısı
ans = [1,2,0,0,4]  # Doğru cevaplar
count = 0  # Kaydedilen görüntü sayacı

# Ana döngü, kullanıcı kapatana kadar devam eder
while True:
    if webCamFeed:
        success, img = cap.read()  # Kameradan görüntüyü oku
    else:
        img = cv2.imread(pathImage)  # Dosyadan görüntüyü oku

    if img is None:
        print("Görüntü yüklenemedi!")
        continue

    img = cv2.resize(img, (widthImg, heightImg))  # Görüntüyü yeniden boyutlandır
    imgFinal = img.copy()  # Görüntüyü son işlem için kopyala
    imgBlank = np.zeros((heightImg, widthImg, 3), np.uint8)  # Test/debug için boş görüntü oluştur
    imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # Görüntüyü griye çevir
    imgBlur = cv2.GaussianBlur(imgGray, (5, 5), 1)  # Gaus bulanıklığı ekle
    imgCanny = cv2.Canny(imgBlur, 10, 70)  # Kenar tespiti yap
    ...
    try:
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            for image in boxes:
                ...
                totalPixels = cv2.countNonZero(image)#iamgedeki işaretli boxları alacapız 5 tane almak yeterli
            ...
            for x in range(0,questions):
                if ans[x] == myIndex[x]:
                    grading.append(1)# answer doğruysa o soru için verdiği cevaba 1 değerini ata, yanlışsa 0
            ...
            imgRawDrawings = np.zeros_like(imgWarpColored) # NEW BLANK IMAGE WITH WARP IMAGE SIZE doğru yanlışların orjinal görüntü üstüne eklenemiş halini göstemrk
            ...
            imgInvGradeDisplay = cv2.warpPerspective(imgRawGrade, 
                                    invMatrixG, (widthImg, heightImg)) # INV IMAGE WARP
            ...
            imageArray = ([img,imgGray,imgCanny,imgContours],
                          [imgBigContour,imgThresh,
                           imgWarpColored,imgFinal])
            cv2.imshow("Final Result", imgFinal)

            if cv2.waitKey(1) & 0xFF == ord('q'):#sonradan ekledim 
                break  # Döngüyü kırarak çıkış yapar
    except:  
        imageArray = ([img,imgGray,imgCanny,imgContours],
